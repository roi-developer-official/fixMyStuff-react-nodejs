...
module.exports.signUp = async (req,res,next)=>{
    ...
    try {
         result = await sequelize.transaction(async (t)=>{
            ...
            if(roleId === 2){;
                professionId = await ProfessionName.scope({method:['findByName',profession]}).findOne({attributes: ['id'],transaction: t});
                experienceId = await Experience.scope({method:['findByName',experience]}).findOne({attributes: ['id'],transaction:t});
